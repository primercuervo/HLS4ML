    ...
    typename CONFIG_T::bias_t    biases[CONFIG_T::n_out])
{
    ...
    if (CONFIG_T::io_type == io_parallel){
        ...
        // #pragma HLS DATAFLOW
        // #pragma HLS STREAM variable=data,mult,acc
    ...
    Product1: for(int ii = 0; ii < CONFIG_T::n_in; ii++) {
        if (CONFIG_T::io_type == io_serial){
            #pragma HLS PIPELINE
            int internal_multiplier_limit = ceil(CONFIG_T::n_out / CONFIG_T::reuse_factor);
            #pragma HLS ALLOCATION instances=mul limit=internal_multiplier_limit operation
        }
        cache = data[ii];
    ...
    Accum1: for(int ii = 0; ii < CONFIG_T::n_in; ii++) {
        if (CONFIG_T::io_type == io_serial){
            #pragma HLS PIPELINE
            // int internal_adder_limit = ceil(CONFIG_T::n_out / CONFIG_T::reuse_factor);
            // #pragma HLS ALLOCATION instances=add limit=internal_adder_limit operation
        }
